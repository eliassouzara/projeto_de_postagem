...
class UserController extends BaseController
{
    ...
    public function do_forgot_password()
    {
        if (Confide::forgotPassword(Input::get('email'))) {
            ...
            return Redirect::action('UserController@forgot_password')->withInput()->with('error', $error_msg);
        }
    }
    /**
    ...
    public function checkCaptcha()
    {
        ...
        $setting = new Setting();
        $publickey = $setting->getSetting('recaptcha_publickey', '');
        // "6LeoOPASAAAAAPsHsCBdbM60dEBKVDydRItjlmHR"; // you got this from the signup page
        $privatekey = $setting->getSetting('recaptcha_privatekey', '');
        //"6LeoOPASAAAAAB_fPJ0h5iOmwp5p-lqldnLk0zjY";
        if ($_POST["recaptcha_response_field"]) {
            ...
            if ($resp->is_valid) {
                ...
                exit("The reCAPTCHA wasn't entered correctly. Go back and try it again." . "(reCAPTCHA said: " . $resp->error . ")");
            }
        } else {
    ...
    public function updateSetting()
    {
        ...
        return Redirect::action('UserController@viewProfile')->with('notice', "Profile updated successfully.");
    }
        ...
        if (trim($withdraw_id) != '' && trim($confirmation_code) != '') {
            ...
            if (isset($withdraw->to_address)) {
                if ($confirmation_code == $withdraw->confirmation_code && $withdraw->status == 0) {
                        ...
                        if ($balance_amount > $withdraw->amount && $admin_balance > $withdraw->amount) {
                            ...
                            if ($txid && $balance->takeMoney($withdraw->amount, $wallet->id, $withdraw->user_id)) {
                                ...
                                if ($txid) {
                                    ...
                                    return Redirect::to('user/withdraw/' . $wallet->id)->with('notice', Lang::get('admin_messages.withdraw_success', array('amount' => sprintf('%.8f', $withdraw->receive_amount), 'coin' => $wallet->type, 'address' => $withdraw->to_address, 'fee' => $withdraw->fee_amount)));
                                }
                            } else {
                                return Redirect::to('user/withdraw/' . $wallet->id)->with('error', "Your balance are not enough.");
                            }
                        } elseif ($admin_balance < $withdraw->amount) {
                            return Redirect::to('user/withdraw/' . $wallet->id)->with('error', "Sorry. Now we are not enough money. Please withdraw later.");
                        } else {
                            return Redirect::to('user/withdraw/' . $wallet->id)->with('error', "Your balance are not enough.");
                        }
                    } catch (Exception $e) {
                    ...
                    return Redirect::to('user/profile/withdrawals')->with('error', "The confirmation code not matching.");
                }
            } else {
    ...
    public function referreredTradeKey()
    {
        ...
        if (isset($user_referred->username) && $user_referred->id != $user->id) {
            ...
            return Redirect::to('user/profile/dashboard')->with('error', 'Sorry. You can not referrer to yourself!');
        } else {
            return Redirect::to('user/profile/dashboard')->with('error', 'Sorry. The trade key not exist!');
        }
    }
