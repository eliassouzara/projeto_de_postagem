...
class Wallet extends Eloquent
{
    ...
    {
        if ($account != '') {
            return $this->jsonRPCclient->getreceivedbyaccount($account);
        } else {
    ...
    {
        if ($account != '') {
            return $this->jsonRPCclient->listtransactions($account);
        } else {
            return $this->jsonRPCclient->listtransactions();
        }
    }
    ...
    {
        $fee = $this->getTxFee();
        return $this->jsonRPCclient->sendtoaddress($address, (double) sprintf("%.8f", $amount));
    }
    ...
    {
        if ($account != '') {
            return $this->jsonRPCclient->getbalance($account);
        } else {
            return $this->jsonRPCclient->getbalance();
        }
    }
