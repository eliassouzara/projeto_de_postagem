...
class WithdrawController extends ApiPublicController
{
    ...
    public function actionBindCard()
    {
        ...
        if ($res) {
            $this->_exit($this->_error['20000'], '20000', $res, 'status');
        }
    ...
    public function actionBindInfo()
    {
        ...
        if (!empty($res)) {
            $this->_exit($this->_error['20000'], '20000', $res, 'info');
        }
    }
    /**
     * 提交提现订单
    ...
    public function actionOrder()
    {
        ...
        if (!isset($_POST['name']) || !$_POST['name']) {
            $this->_exit($this->_error['20041'], '20041');
        }
        if (!isset($_POST['city']) || !$_POST['city']) {
            $this->_exit($this->_error['20042'], '20042');
        }
        if (!isset($_POST['bank']) || !$_POST['bank']) {
            $this->_exit($this->_error['20043'], '20043');
        }
        if (!isset($_POST['bname']) || !$_POST['bname']) {
            $this->_exit($this->_error['20044'], '20044');
        }
        if (!isset($_POST['bcard']) || !$_POST['bcard']) {
            $this->_exit($this->_error['20045'], '20045');
        }
        if (!isset($_POST['coins']) || !$_POST['coins']) {
            $this->_exit($this->_error['20047'], '20047');
        }
        if (!isset($_POST['totalcoin'])) {
            $this->_exit($this->_error['20050'], '20050');
        }
        $data = array();
        ...
        if ($data['coins'] > $data['totalcoin']) {
            $this->_exit($this->_error['20051'], '20051');
        }
        if ($data['coins'] % 10 != 0) {
            $this->_exit($this->_error['20048'], '20048');
        }
        $data['money'] = $data['coins'];
        //比例暂时1:1
        $status = Withdraw::model()->getBindStatus($data['uid']);
        if ($status == 2) {
            $res = Withdraw::model()->insertCardInfo($data);
        } else {
            $res = 1;
        }
